...
def _assemble_laplacian(eps_r: np.ndarray, dx: float):
    """
...
def solve_poisson(
    ...
    """
    N = len(eps_r)
    eps_m, eps_p = _assemble_laplacian(eps_r, dx)
    ...
    # Equation: [(eps_m+eps_p)/dx² * phi_i - eps_m/dx² * phi_{i-1} - eps_p/dx² * phi_{i+1}] = rho_i/eps0
    # Equivalent to: d/dz[eps_r dphi/dz] = -rho/eps0  (correct Poisson)
    diag_main[1:-1]  =  (eps_m + eps_p) / dx**2
    diag_lower[:-1]  = -eps_m / dx**2        # A[i, i-1] for i=1..N-2
    diag_upper[1:]   = -eps_p / dx**2        # A[i, i+1] for i=1..N-2
...
def solve_poisson_newton(
    ...
    # Interior: [A_pos + D] * phi_new = D * phi + rho/eps0
    diag_main[1:-1]  = (eps_m + eps_p) / dx**2 + D_int
    diag_lower[:-1]  = -eps_m / dx**2
    diag_upper[1:]   = -eps_p / dx**2
